...
struct RealNewsView: View {
    @StateObject private var rssManager = RealRSSManager.shared
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                ...
                if rssManager.isLoading {
                    VStack(spacing: 16) {
                        ...
                        Text("Fetching from Wrestling Observer, PWTorch, Fightful, and more")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    ...
                    List(filteredArticles) { article in
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            if !article.promotions.isEmpty {
                                HStack {
                                    ForEach(article.promotions, id: \.self) { promotion in
        ...
        .alert("Error", isPresented: $showingError) {
            ...
            if let error = rssManager.error {
                Text(error.localizedDescription)
            }
        }
        .onChange(of: rssManager.error) { error in
            if error != nil {
                showingError = true
            }
        }
